...
export async function GET(req, { params }) {
  try {
    ...
    console.log("API Request for ID:", resolvedParams.id);
    ...
    const whiteboard = await Whiteboard.findById(resolvedParams.id)
      .populate("user", "username email")
      .populate("collaborators", "username email");

    if (!whiteboard) {
      console.log("Whiteboard not found for ID:", resolvedParams.id);
...
export async function PUT(request, { params }) {
  try {
    ...
    const whiteboardId = resolvedParams.id
    ...
    const now = new Date()
    const lastModified = new Date(whiteboard.lastModified)
    const timeDiff = (now.getTime() - lastModified.getTime()) / 1000 // in seconds
    
    if (timeDiff < 60) {
      const remainingTime = Math.ceil(60 - timeDiff)
      return NextResponse.json({ 
        error: `Please wait ${remainingTime} seconds before saving again` 
      }, { status: 429 })
    }
